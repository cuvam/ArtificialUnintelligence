...
# DEFLATE Compression Test Suite
# Tests the DEFLATE compression utility with various test cases
...
DEFLATE_BINARY="$SCRIPT_DIR/deflate"
TEST_DIR="/tmp/deflate_tests_$$"

# Compile the DEFLATE program if needed
if [ ! -f "$DEFLATE_BINARY" ]; then
    echo -e "${YELLOW}Compiling DEFLATE compression program with optimizations...${NC}"
    gcc -o "$DEFLATE_BINARY" "$SCRIPT_DIR/deflate.c" -Wall -O3 -march=native
...
run_compression_test() {
    ...
    local output_file="$TEST_DIR/output.deflate"

    # Run compression
    if "$DEFLATE_BINARY" "$input_file" "$output_file" > "$TEST_DIR/deflate_output.txt" 2>&1; then
...
echo -e "${BLUE}================================${NC}"
echo -e "${BLUE}DEFLATE COMPRESSION TEST SUITE${NC}"
echo -e "${BLUE}================================${NC}\n"
...
# TEST 4: Classic compression test string
echo -e "${YELLOW}Test 4: Classic test string${NC}"
echo -n "TOBEORNOTTOBEORTOBEORNOT" > "$TEST_DIR/classic.txt"
run_compression_test "Classic test string" "$TEST_DIR/classic.txt" "should_compress"
...
# TEST 15: Test with actual source code
echo -e "${YELLOW}Test 15: Source code file${NC}"
if [ -f "$SCRIPT_DIR/deflate.c" ]; then
    run_compression_test "DEFLATE source code" "$SCRIPT_DIR/deflate.c" "should_compress"
fi
